 ...
 */
class BukkitCommandHandler(val exceptionParser: IExceptionParser = EnglishExceptionParser, val typeParser: ITypeParser = DefaultTypeParser) : CommandExecutor, TabCompleter {
    ...
    override fun onTabComplete(commandSender: CommandSender?, bukkitCommand: org.bukkit.command.Command?, commandName: String, parameters: Array<String>): MutableList<String> {
        val command = getCommandByName(commandName) ?: throw IllegalArgumentException("BukkitCommandHandler was registered for the command $commandName, but there is no command with that name...")
